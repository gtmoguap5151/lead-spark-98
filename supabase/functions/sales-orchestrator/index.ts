...
Deno.serve(async (request) => {
  ...
  if (secretError || !expectedSecret || suppliedSecret !== expectedSecret) {
  ...
  for (const enrollment of enrollments) {
    ...
    if (!prospect || prospect.opted_out || prospect.stage === "do_not_contact") {
      await supabase.from("sales_enrollments").update({ status: "opted_out", completed_at: now }).eq("id", enrollment.id);
      skipped++;
      continue;
    }
    ...
    if (prospect.email) {
      ...
      if (suppressionError) {
        await supabase.from("sales_enrollments").update({ last_error: suppressionError.message }).eq("id", enrollment.id);
        continue;
      }
      if (suppressions?.length) {
        await supabase.from("sales_prospects").update({ opted_out: true, stage: "do_not_contact" }).eq("id", prospect.id);
        await supabase.from("sales_enrollments").update({ status: "opted_out", completed_at: now, last_error: null }).eq("id", enrollment.id);
        skipped++;
        continue;
      }
    }
    ...
    if (stepError) {
      await supabase.from("sales_enrollments").update({ last_error: stepError.message }).eq("id", enrollment.id);
      continue;
    }
    if (!step) {
      await supabase.from("sales_enrollments").update({ status: "completed", completed_at: now }).eq("id", enrollment.id);
      continue;
    }
    ...
    if (existingError) {
      await supabase.from("sales_enrollments").update({ last_error: existingError.message }).eq("id", enrollment.id);
      continue;
    }

    if (existingActivity) {
      if (existingActivity.status === "sent") {
    ...
    if (activityError) {
      await supabase.from("sales_enrollments").update({ last_error: activityError.message }).eq("id", enrollment.id);
      continue;
    }
