...
function PlumbingLeadsPage() {
  ...
  return <div className="min-h-screen bg-background"><SiteHeader /><main>
    <section className="bg-steel text-steel-foreground"><div className="mx-auto max-w-6xl px-4 py-14 sm:py-20"><p className="text-base font-bold uppercase tracking-wide text-primary">Plumbing leads for contractors</p><h1 className="mt-4 max-w-4xl font-display text-4xl font-bold leading-tight sm:text-6xl">Your first matched plumbing lead is on us.</h1><p className="mt-6 max-w-3xl text-lg leading-relaxed text-steel-foreground/85 sm:text-xl">Create your account and set your plumbing territory. When a genuine homeowner plumbing request matches your coverage, your first eligible lead is free. No card required for that first lead.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button asChild size="lg" className="h-14 px-7 text-lg font-bold"><Link to="/login" search={{ mode: "signup", service: "Plumbing" }}>Claim My First Plumbing Lead</Link></Button><Button asChild size="lg" variant="outline" className="h-14 border-steel-foreground/40 bg-background/10 px-7 text-lg font-semibold text-steel-foreground hover:bg-steel-foreground/10 hover:text-steel-foreground"><Link to="/find-a-plumber">I Need a Plumber</Link></Button></div></div></section>
    ...
    <section className="mx-auto max-w-4xl px-4 py-14 text-center sm:py-16"><h2 className="text-3xl font-bold sm:text-4xl">Ready to add Rivet Reach to your plumbing sales pipeline?</h2><Button asChild size="lg" className="mt-7 h-14 px-8 text-lg font-bold"><Link to="/login" search={{ mode: "signup", service: "Plumbing" }}>Claim My First Plumbing Lead</Link></Button></section>
  </main><SiteFooter /></div>;
}
