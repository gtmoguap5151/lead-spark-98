...
function ProfilePage() {
  const { updateContractor, hydrated, busy } = useApp();
  const contractor = useCurrentContractor();

  return (
    <AppShell title="Profile" subtitle="Your trade, territory & account settings">
      {contractor ? (
        <ProfileForm
          key={contractor.id}
...
function ProfileForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({});
    const result = await onSave(contractor.id, {
      ...parsed.data,
      active,
    });
    if (!result.ok) {
            ...
            {SERVICE_TYPES.map((s) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setServices((cur) => (isActive ? cur.filter((c) => c !== s) : [...cur, s]))
                  }
                  className={cn(
